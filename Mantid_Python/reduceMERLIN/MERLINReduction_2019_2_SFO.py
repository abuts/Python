...
#---------------------------------------------------------------------------#
      ...
      # multiple energies provided in the data file
       ei=[10]#[50,19,10] #[150,64,36] # multiple energies provided in the data file
       ...
       prop['sample_run'] = 47406#range(47462,47700) #range(24003,24011) # 'MER23700.n001'
       prop['wb_run'] = 46493
       #
...
#------------------------------------------------------------------------------------#
   ...
   def def_advanced_properties(self):
      ...
      """
      prop = {}
      prop['map_file'] = 'One2one_192.map' #'one2one_164.map'
      prop['det_cal_file'] = 'det_corr_192_process_5V2.dat' #'det_corrected7.nxs - testing'
...
##### Here one sets up folders where to find input data and where to save results ####
    ...
    map_mask_dir = r'c:\mprogs\InstrumentFiles\merlin'
    config.appendDataSearchDir(map_mask_dir)
    config.appendDataSearchDir(run_dir)
...
#### Set up time interval (sec) for reducer to check for input data file.         ####
    ...
    rd.wait_for_file = 700  # waiting time interval in seconds
