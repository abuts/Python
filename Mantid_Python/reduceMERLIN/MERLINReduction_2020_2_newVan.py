...
#---------------------------------------------------------------------------#
      ...
      # multiple energies provided in the data file
       ei=[70,33.2] #[150,64,36] # multiple energies provided in the data file
       ...
       #filenumber=list(range(53286,53570))
       #filenumber.extend(list(range(53574,53996)))
       #filenumber.extend(list(range(53998,54139)))
       #filenumber.extend(list(range(54141,54422)))
       #prop['sample_run'] = range(53790,53791) # 'MER23700.n001'
       prop['sample_run'] = 53790
       prop['wb_run'] = '50250.nxs'
       #
...
#------------------------------------------------------------------------------------#
   ...
   def def_advanced_properties(self):
      ...
      prop['det_cal_file'] = 'det_corr_202_process_5.dat' #'det_corrected7.nxs - testing'
      prop['norm_method']='current' #'monitor-1', 'monitor-2'
      prop['detector_van_range']=[20,100]
      prop['background_range'] = [18000,19000] # TOF range for the calculating flat background
      prop['hardmaskPlus']='Mask_192.xml' # Use diag (hardmaskPlus option) to enhance hard masks
      ...
      prop['monovan_mapfile'] = 'rings_192.map'
...
#------------------------------------------------------------------------------------#
   ...
   def reduce(self,input_file=None,output_directory=None):
        ...
        """
        Load(Filename='MER53790.nxs', OutputWorkspace='sws', LoadMonitors=True)
        Rebin(InputWorkspace='sws_monitors', OutputWorkspace='sws_monitors', Params='100,5,20000')
        sws = mtd['sws']
        mon_ws = mtd['sws_monitors']
        sws.monitorWorkspace = mon_ws
        mon_ws1 = sws.getMonitorWorkspace()
        results = ReductionWrapper.reduce(self,sws,output_directory)
   ...
   def set_custom_output_filename(self):
      ...
      def custom_name(prop_man):
            ...
            name = "spe/MER{0}_Ei{1:<3.2f}meV_One2One".format(run_num ,ei)
            return name
...
#### Set up time interval (sec) for reducer to check for input data file.         ####
    ...
    rd.wait_for_file = 0#600  # waiting time interval in seconds
