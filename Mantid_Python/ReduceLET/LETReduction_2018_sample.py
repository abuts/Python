...
#---------------------------------------------------------------------------#
      ...
      # prop['sample_run'] = range(50359,52000) # 'LET18547.n001'
       prop['sample_run']=8585 #range(50776,50793) #SET DATA RUN NUMBERS
...
#------------------------------------------------------------------------------------#
   ...
   def def_advanced_properties(self):
      ...
      prop['data_file_ext']='.raw' # for LET it may be choice between event and histo mode if 
      # raw file is written in histo, and nxs -- in event mode
      # Request the file extension exactly as specified by the property above. Fail or wait if raw file find instead of or and v.v.
      prop['fix_file_extension']= False      
...
#### Set up time interval (sec) for reducer to check for input data file.         ####
    ...
    rd.wait_for_file = 10  # waiting time interval in seconds
